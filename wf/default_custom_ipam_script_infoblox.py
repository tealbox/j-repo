...
def _verify_required_fields_in_auth_params(auth_params):
    ...
    logger.info("Inside F[_verify_required_fields_in_auth_params] auth_params[%s]", auth_params)
    missing_req_params = []
...
def TestLogin(auth_params):
    ...
    logger.info("Inside F[TestLogin] auth_params[%s]", auth_params)
    ...
    logger.info("F[iphone] req[%s] status_code[%s]" % (schema_url, r.status_code))
...
def GetAvailableNetworksAndSubnets(auth_params, ip_type):
    ...
    logger.info("Tao Inside F[GetAvailableNetworksAndSubnets] auth_params[%s] and ip_type[%s]", auth_params, ip_type)
    network_view = auth_params.get('network_view', 'default')
    if 'server' in auth_params:
    ...
    logger.info("Tao Return F[GetAvailableNetworksAndSubnets] subnet_list[%s]", subnet_list)
    return subnet_list
...
def CreateIpamRecord(auth_params, record_info):
    ...
    logger.info("Tao Inside F[CreateIpamRecord] auth_params[%s] and record_info[%s]", auth_params, record_info)
    network_view = auth_params.get('network_view', 'default')
    ...
    else: ## not a V4_V6 so could be v4_subnet or v6_subnet
        for sub in nw_and_subnet_list:
            ...
            if 'server6' in auth_params:
                ...
                if r6.status_code in [200, 201]:
                    ...
                    if 'ipv6addrs' in r6_json and r6_json['ipv6addrs'][0]['ipv6addr']:
                        ...
                        alloc_info['v6_subnet'] = v6_subnet
                    return alloc_info
                elif 'server' in  auth_params:  ### no server6 so this will execute
                    r = requests.post(url=rest_url, auth=(auth_params['username'], auth_params['password']),
                                    verify=False, data=payload)
                    logger.info("F[CreateIpamRecord] req[%s %s] status_code[%s]" % (rest_url, payload, r.status_code))
                    _check_and_raise_auth_error(r)
                    if r.status_code in [200,201]:
                        ...
                        if 'ipv6addrs' in r_json and r_json['ipv6addrs'][0]['ipv6addr']:
                            alloc_info['v6_ip'] = r_json['ipv6addrs'][0]['ipv6addr']
                            alloc_info['v6_subnet'] = v6_subnet
                        logger.info("Tao Return F[CreateIpamRecord] alloc_info[%s]", alloc_info)
                        return alloc_info
                    else:
                        emsg = "req[%s %s], rsp[%s, %s]" % (rest_url, payload, r.status_code, r.text)
                        err_msgs.append(emsg)
                    ...
                    err_msgs.append(emsg)
                    continue
            elif 'server' in auth_params: ### Section 2
                r = requests.post(url=rest_url, auth=(auth_params['username'], auth_params['password']),
                            verify=False, data=payload)
                logger.info("F[CreateIpamRecord] req[%s %s] status_code[%s]" % (rest_url, payload, r.status_code))
                _check_and_raise_auth_error(r)
                if r.status_code in [200,201]:
                    ...
                    if 'ipv6addrs' in r_json and r_json['ipv6addrs'][0]['ipv6addr']:
                        alloc_info['v6_ip'] = r_json['ipv6addrs'][0]['ipv6addr']
                        alloc_info['v6_subnet'] = v6_subnet
                    logger.info("Tao Return [Section 2] F[CreateIpamRecord] alloc_info[%s]", alloc_info)
                    return alloc_info
                else:
                    emsg = "req[%s %s], rsp[%s, %s]" % (rest_url, payload, r.status_code, r.text)
                    err_msgs.append(emsg)
...
def DeleteIpamRecord(auth_params, record_info):
    ...
    logger.info("Tao Inside F[DeleteIpamRecord] auth_params[%s] and record_info[%s]", auth_params, record_info)
    id = record_info.get('id', None)
    fqdns = record_info.get('fqdns', None)
    name = id
