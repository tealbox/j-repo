...
def _verify_required_fields_in_auth_params(auth_params):
    ...
    logger.info("Tao Inside DNS F[_verify_required_fields_in_auth_params] auth_params: [%s]", auth_params)
    missing_req_params = []
...
def _create_dns_record(auth_params, record_name, ips):
    ...
    try:
        if server6:
            ...
            r_json = r.json()
            if r.status_code == 200 or r.status_code == 201:
                logger.info("Tao Inside DNS F[_create_dns_record] r_json: [%s]", r_json)
                return
            elif r.text and 'already exists' in r_json['text']:
                host_ips = _get_ips_by_host(auth_params, record_name)
                logger.info("record_name[%s], record ips on infoblox[%s]" %(record_name, host_ips))
                    ...
                    if expected_ips is not None:
                        ...
                        if expected_v4_ips or expected_v6_ips:
                            ...
                            if not remaining_v4_ips and not remaining_v6_ips:
                                ...
                                r_json = r.json()
                                if r.status_code == 200 or r.status_code == 201:
                                    logger.info("Tao Inside DNS F[_delete_dns_record] r_json: [%s] & payload: [%s]", r_json, payload)
                                    return
                                else:
                                    if 'text' in r_json:
                                        err_msg = str(r.status_code) + (' : ' + r_json['text'] if 'text' in r_json else '')
...
def TestLogin(auth_params):
    ...
    logger.info("Tao Inside DNS F[TestLogin] auth_params: [%s]", auth_params)
    server = auth_params.get('server', None)
    server6 = auth_params.get('server6', None)
    username = auth_params.get('username', None)
    password = auth_params.get('password', None)
...
def DeleteDnsRecords(records_info, auth_params):
    ...
    logger.info("Tao Inside DNS F[DeleteDnsRecords] records_info: [%s], auth_params: [%s]", records_info, auth_params)
...
def CreateOrUpdateDnsRecords(new_records_info, old_records_info, auth_params):
    ...
    logger.info("Tao Inside DNS F[CreateOrUpdateDnsRecords] auth_params: [%s], new_records_info: [%s], old_records_info: [%s]", auth_params, new_records_info, old_records_info)
    ...
    if not old_records_info:
        for fqdn in new_records_info['dns_info'].keys():
            logger.info("Tao Inside DNS F[CreateOrUpdateDnsRecords] old record empty auth_params: [%s], new_records_info: [%s], old_records_info: [%s]", auth_params, new_records_info, old_records_info)
