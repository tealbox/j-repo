...
def get_name_to_switch_dict_by_fabric(nd_rest_client, fabric_name):
    rest_resp = nd_rest_client.nd_rest_req(
        api_endpoint_uri=f"/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/control/fabrics/{fabric_name}/inventory/switchesByFabric",
        payload_data={},
        method="GET",
    ...
    name_to_switch = {}
    for switch in switches:
        name_to_switch[switch["logicalName"]] = switch["serialNumber"]
    return name_to_switch

def get_links_to_switch(nd_rest_client, fabric_name, switch_name):
    name_to_serial = get_name_to_switch_dict_by_fabric(nd_rest_client,fabric_name)
    sr = convert_switch_to_serial(switch_name,name_to_serial)
    links_to_switch = []
    if sr != "":
        rest_resp = nd_rest_client.nd_rest_req(
            api_endpoint_uri=f"/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/control/links/switch/{sr}",
            payload_data={},
            method="GET",
        )
        links = rest_resp.json()

        for link in links:
            if "policyId" in link:
                links_to_switch.append(link)
        return links_to_switch
    else:
        return ""

def get_link_details(nd_rest_client, linkId):
    rest_resp = nd_rest_client.nd_rest_req(
        api_endpoint_uri=f"/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/control/links/{linkId}",
        payload_data={},
        method="GET",
    )
    links = rest_resp.json()
    if links:
        ...
        return ""
##https://10.10.20.60/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/control/links/LINK-UUID-33820
...
def convert_switch_to_serial(switch_name, serial_to_switch):
    for switch, serial  in serial_to_switch.items():
        if switch == switch_name:
            return serial
    return ""
...
##    data_from_file = read_from_yml(yml_file_path)
    nd_rest_client = NDRESTClient(
        base_uri="https://10.10.20.60", username="admin", pwd="1vtG@lw@y"
    )
    # Login to ND
    login = nd_rest_client.nd_login()
    ...
    print(get_name_to_switch_dict_by_fabric(nd_rest_client,fabric_name))

    links_to_switch = get_links_to_switch(nd_rest_client, fabric_name, "leaf1")

    for link in links_to_switch:
        print(link['link-uuid'], link['policyId'])


    d = get_link_details(nd_rest_client, "LINK-UUID-33800")
    json.dumps(d)
